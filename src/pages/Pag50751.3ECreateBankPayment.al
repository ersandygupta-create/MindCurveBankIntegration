page 50751 "Create Bank Payment"
{
    ...
    var
        ...
        BankIntegration: Codeunit "3E Bank Integration";
    begin
    ...
    protected var NextDocNo: Code[20];
    JournalBatchName: Code[10];
    JournalTemplateName: Code[10];
    BankPaymentType: Enum "Bank Payment Type";
    procedure GetPostingDate(): Date begin
        exit(PostingDate);
    end;
    procedure GetBankAccount(): Text begin
        exit(Format(BalAccountNo));
    end;
    procedure GetBankPaymentType(): Integer begin
        exit(BankPaymentType.AsInteger());
    end;
    procedure GetBatchNumber(): Code[10]begin
        exit(JournalBatchName);
    end;
    procedure GetTemplateName(): Code[10]begin
    ...
    begin
        ...
        VendorLedgerEntry.SetRange("Payment Created", false);
        if VendorLedgerEntry.Find('-')then repeat if Vendor.Get(VendorLedgerEntry."Vendor No.")then Vendor.CheckBlockedVendOnJnls(Vendor, GenJournalDocType::Payment, true);
                if PostingDate < VendorLedgerEntry."Posting Date" then Error(EarlierPostingDateErr, VendorLedgerEntry."Document Type", VendorLedgerEntry."Document No.");
                ...
                if VendorLedgerEntry."Applies-to ID" = '' then begin
                    ...
                    if VendorLedgerEntry."Payment Method Code" = '' then begin
                        if Vendor.Get(VendorLedgerEntry."Vendor No.")then TempVendorPaymentBuffer."Payment Method Code":=Vendor."Payment Method Code";
                    end
                    else
                    ...
                    if CheckCalcPmtDiscGenJnlVend(VendorLedgerEntry."Remaining Amount", VendorLedgerEntry, 0, false)then PaymentAmt:=-(VendorLedgerEntry."Remaining Amount" - VendorLedgerEntry.GetRemainingPmtDiscPossible(PostingDate))
                    else
                    ...
                    Error(PaymentApplicationInProcessErr, VendorLedgerEntry."Applies-to ID", VendorLedgerEntry."Entry No.")until VendorLedgerEntry.Next() = 0;
        if TempVendorPaymentBuffer.IsEmpty()then Error(ThereAreNoPaymentsToProccesErr);
    ...
    begin
        ...
        if TempVendorPaymentBuffer.Find('-')then repeat GenJnlLine.Init();
                ...
                if TempVendorPaymentBuffer."Vendor Ledg. Entry Doc. Type" = TempVendorPaymentBuffer."Vendor Ledg. Entry Doc. Type"::Invoice then GenJnlLine."Document Type":=GenJnlLine."Document Type"::Payment
                else if TempVendorPaymentBuffer.Amount > 0 then GenJnlLine."Document Type":=GenJnlLine."Document Type"::Payment
                    else
    ...
    begin
        if(GenJournalBatchNoSeries = '')then begin
            if not KeepSavedDocumentNo then NextDocNo:='' end
        else
        begin
            GenJournalLine.SetRange("Journal Template Name", JournalTemplateName);
            GenJournalLine.SetRange("Journal Batch Name", JournalBatchName);
            if GenJournalLine.FindLast()then NextDocNo:=IncStr(GenJournalLine."Document No.")
            else
                NextDocNo:=NoSeriesBatch.GetNextNo(GenJournalBatchNoSeries, PostingDate, true);
        end;
    end;
    procedure CheckCalcPmtDiscGenJnlVend(RemainingAmt: Decimal; OldVendLedgEntry2: Record "Vendor Ledger Entry"; ApplnRoundingPrecision: Decimal; CheckAmount: Boolean): Boolean var
    ...
    local procedure GetMessageToRecipient(TempVendorPaymentBuffer: Record "Vendor Payment Buffer" temporary; DocumentsToApply: List of[Text]): Text[140]var
